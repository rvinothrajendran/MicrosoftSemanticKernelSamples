...
public class RootDialog : ComponentDialog
{
        ...
        CancellationToken cancellationToken)
    {
       ...
       var result = await skHelper.ProcessAsync(text, movieTemplate, intentList, cancellationToken);


        var attachments = new List<Attachment>();

        if (result.YouTubeLinks?.Count >= 0)
        {
            foreach (var videoUrl in result.YouTubeLinks)
            {
        ...
        {
            return await stepContext.EndDialogAsync(MessageFactory.Text("No videos found"), cancellationToken);
        }
